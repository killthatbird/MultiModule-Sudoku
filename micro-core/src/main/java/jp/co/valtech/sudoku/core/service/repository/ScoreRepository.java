...
/**
 * SCORE_INFO_TBLへのRepositoryクラスです。
...
public interface ScoreRepository extends JpaRepository<ScoreInfoTbl, Long> {
	 ...
	 * @since 1.0
	 */
	@Query(value = "SELECT * FROM SCORE_INFO_TBL INNER JOIN ANSWER_INFO_TBL ON SCORE_INFO_TBL.NO = ANSWER_INFO_TBL.NO WHERE ANSWER_INFO_TBL.TYPE = :TYPE AND ANSWER_INFO_TBL.KEYHASH = :KEYHASH", nativeQuery = true)
